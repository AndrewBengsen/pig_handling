...
As expected, there's no real difference in induction time between pole-jabbed and darted pigs. There's also no difference by sex within treatments (not shown here).       


```{r plot induction time by capture method}
it_plot <- ggsurvplot(it1, data=s, conf.int = T, 
                       palette = jama_palette3,
                       legend = "right", legend.title = "Capture method",
...
As with induction, there were no differences between sex within treatments (not shown here).  
...
Once again, no difference between sexes within treatments.  
...
#ggsave("output/Fig 4.pdf", all_plot, height = 15, width = 9, units = "cm")
```
