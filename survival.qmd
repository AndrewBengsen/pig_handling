...
## latent time for manually restrained pigs  

```{r latent time for manual, include = T}

mr <- filter(s, method == "Manual") %>%
  mutate(method = "Manual (LT)")

hist(mr$latent_min)
range(mr$latent_min, na.rm=T)
median(mr$latent_min, na.rm=T)
table(mr$latent_min)

surv.lt <- Surv(time = mr$latent_min, event = rep(1, nrow(mr)))

# KM survival model 
lt_mr <- survfit(surv.lt ~ 1, data=mr)
lt_mr
...
length(which(mr$latent_min > 30)) / nrow(mr)

```


```{r plot latent time for manual}
lt_plot <- ggsurvplot(lt_mr, data=mr, conf.int = T, 
                       palette = 1,
                       legend = "none", 
                      linetype = 2,
  ...
  xlab("latent time (mins)") +
  ylab("Proportion of pigs")

(lt_plot2 <- lt_plot$plot +
...
s %>%
  ...
  summarise(n = n(),
            ...
            n_over_60 = length(which(rt > 60))) %>%
  mutate(p_over_60 = n_over_60 / n *100) %>%
  knitr::kable(digits=0)

```

What proportion of pigs took more than 1 hour to recover?  
...
tt_plot <- ggsurvplot(tt1, data=s, conf.int = T, 
           palette = jama_palette3,
           legend = "bottom", legend.title = "Program",
           legend.labs = c("A (Dart)", "B (Pole\nsyringe)", "C (Manual\nrestraint)"),
           ggtheme = theme_survminer(base_size = 9,
                                     font.caption = c(9, "plain", "black"),
                                     font.x = c(9, "plain", "black"),
                                     font.tickslab = c(9, "plain", "black"),
...
write.csv(s, "output/revised_time-to-event_data.csv", row.names = F)
```


Now include latent period for manually restrained pigs.


```{r total time, include = T}
# survival formula
s_latent <- rbind(s, mr)
s_latent$total_min[which(s_latent$method == "Manual (LT)")] <- s_latent$latent_min[which(s_latent$method == "Manual (LT)")] + s_latent$rt[which(s_latent$method == "Manual (LT)")]

surv.ttL <- Surv(time = s_latent$total_min, event = rep(1, nrow(s_latent)))

# KM survival model by method
ttL <- survfit(surv.ttL ~ method, data=s_latent)
ttL
...
```{r plot capture method TTL}
ttL_plot <- ggsurvplot(ttL, data=s_latent, conf.int = T, 
                       palette = c(jama_palette3, 1),
                       linetype = c(1,1,1,3),
                       legend = "bottom", legend.title = "Program",
                       legend.labs = c("A (Dart)", "B (Pole syringe)", "C (Manual restraint)", "C (Manual restraint\n with LT)"),
                       ggtheme = theme_survminer(base_size = 9,
                                                 font.caption = c(9, "plain", "black"),
                                                 font.x = c(9, "plain", "black"),
                                                 font.tickslab = c(9, "plain", "black"),
...
```{r plot all curves TTL}
all_plotLT <- plot_grid(it_plot2, rt_plot2, tt_plot3, ncol=1,
                      rel_heights = c(0.30, 0.30, 0.40), 
                      labels = c("a)", "b)", "c)"), label_x = 0.9, label_size = 10)
all_plotLT

ggsave("output/survival_plot_panel_LT.png", all_plotLT, height = 15, width = 9, units = "cm")
ggsave("output/Fig 4.pdf", all_plotLT, height = 15, width = 9, units = "cm")
```


What's the minimum TT for each method?

```{r }
s_latent %>%
  group_by(method) %>%
  summarise(min_tt = min(total_min, na.rm=T),
            max_tt = max(total_min, na.rm=T))
```


Did long latent periods coincide with large numbers of pigs in traps? Which pigs had latent > 15 and did those same pigs also have lots of friends in the trap?

```{r }
long_latent <- s_latent$id[which(s_latent$latent_min > 30)]

ll <- read.csv("input/working_pig_data_2025-05-30.csv") %>%
  janitor::clean_names() %>%
  filter(id %in% long_latent) %>%
  select(id, n_in_trap)
ll
...
s |>
  group_by(method) |>
  summarise(lt_mean = mean(latent_min, na.rm=T),
            lt_sd = sd(latent_min, na.rm=T),
            it_mean = mean(induction_min, na.rm=T),
            it_sd = sd(induction_min, na.rm=T),
            rt_mean = mean(rt, na.rm=T),
            rt_sd = sd(rt, na.rm=T),
            tt_mean = mean(total_min, na.rm=T),
            tt_sd = sd(total_min, na.rm=T),
            tt_latent_mean = mean(total_min + latent_min, na.rm=T), 
            tt_latent_sd = sd(total_min + latent_min, na.rm=T)) |>
  knitr::kable()
```
